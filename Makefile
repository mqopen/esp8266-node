...
ifeq ($(KBUILD_EXTMOD),)
firmware-dirs	:= $(patsubst %/,%,$(filter %/, $(core-y) $(core-m) $(libs-y) $(libs-m)))

firmware-alldirs	:= $(sort $(firmware-dirs) $(patsubst %/,%,$(filter %/, \
...
firmware-all  := $(core-y) $(libs-y)

# Rule to link busybox - also used during CONFIG_KALLSYMS
# May be overridden by arch/$(ARCH)/Makefile
quiet_cmd_firmware__ ?= LINK    $@
      cmd_firmware__ ?= $(srctree)/scripts/trylink \
      "$@" \
      "$(CC)" \
...
define rule_firmware__
	:
	$(call cmd,firmware__)
	$(Q)echo 'cmd_$@ := $(cmd_firmware__)' > $(@D)/.$(@F).cmd
endef
...
.tmp_busybox1: $(firmware-lds) $(firmware-all) FORCE
	$(call if_changed_rule,ksym_ld)

.tmp_busybox2: $(firmware-lds) $(firmware-all) .tmp_kallsyms1.o FORCE
	$(call if_changed,firmware__)

.tmp_busybox3: $(firmware-lds) $(firmware-all) .tmp_kallsyms2.o FORCE
	$(call if_changed,firmware__)
...
firmware_unstripped: $(firmware-all) FORCE
	$(call if_changed_rule,firmware__)
	$(call if_changed_rule,firmware__)
	$(Q)rm -f .old_version

#firmware: busybox_unstripped
#firmware: firmware_unstripped
#ifeq ($(SKIP_STRIP),y)
#	$(Q)cp $< $@
#else
#	$(Q)$(STRIP) -s --remove-section=.note --remove-section=.comment \
#		firmware_unstripped -o $@
## strip is confused by PIE executable and does not set exec bits
#	$(Q)chmod a+x $@
#endif

firmware.elf: $(firmware-all)
	$(LD) -Wl,--start-group $(addprefix -l,$(LDLIBS)) -Wl,--end-group -lgcc -Teagle.app.v6.ld -o $@ $^
...
$(sort $(firmware-all)): $(firmware-dirs) ;
...
PHONY += modules
modules: $(firmware-dirs) $(if $(KBUILD_BUILTIN),firmware)
...
CLEAN_FILES +=	firmware firmware_unstripped* firmware.links \
...
clean-dirs      := $(addprefix _clean_,$(srctree) $(firmware-alldirs))
...
PHONY += doc-clean
doc-clean: rm-files := docs/firmware.pod \
		  docs/BusyBox.html docs/firmware.1 docs/BusyBox.txt
doc-clean:
	$(call cmd,rmfiles)
...
PHONY += checkstack
checkstack:
	$(OBJDUMP) -d firmware $$(find . -name '*.ko') | \
